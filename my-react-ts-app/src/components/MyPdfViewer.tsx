...
const previewPdf = async () => {
  try {
    ...
    // Map sections to LaTeX format
    const summarySection = summarys.map(summary => `\\textit{${summary.content}}`).join("\n");
    ...
    const involvementSection = involvements.map(involvement => `
    ...
    const certificationSection = certifications.map(certification => `
        \\begin{bullet-list-major}
    ...
    const latexCode = `
      ...
      \\begin{center}
        ...
        \\faEnvelope\\ \\href{mailto:${
          ...
          ? contacts[0].email
          : `${userDetails?.email}`
      }}{${contacts[0]?.email || userDetails?.email}} \\quad
